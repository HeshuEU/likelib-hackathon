...
BOOST_AUTO_TEST_CASE(time_default_constructor)
{
    ...
    BOOST_CHECK(time1.getSecondsSinceEpoch() == 0);
    BOOST_CHECK(time2.getSecondsSinceEpoch() == 0);
}
...
BOOST_AUTO_TEST_CASE(time_from_TimePoint1)
{
    ...
    BOOST_CHECK(time1.getSecondsSinceEpoch() == time2.getSecondsSinceEpoch());
...
{
    base::Time time1(base::Time::now());
    auto seconds = time1.getSecondsSinceEpoch();
...
BOOST_AUTO_TEST_CASE(time_seconds_serialization)
{
    ...
    auto time_1_serialized{ now.getSecondsSinceEpoch() };
    auto now_2{ base::Time(time_1_serialized) };
    auto time_2_serialized{ now_2.getSecondsSinceEpoch() };
