...
BOOST_AUTO_TEST_CASE(inner_json_parse)
{
    ...
    base::json::Value result = base::json::Value::parse(json);
    BOOST_CHECK_EQUAL(result["result"].as_object()["top_block_number"].as_number().to_uint64(), 1u);
}
