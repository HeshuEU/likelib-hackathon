...
base::Result<int> someFunction(int x)
{
    if(x % 2 == 0) {
        ...
        return base::Error{base::ErrorCode::INVALID_ARGUMENT, "is not even"};
    }
}
...
{
    if(gonna_fail) {
        return base::Error(base::ErrorCode::INVALID_ARGUMENT, "set to fail");
    }
    else {
...
base::Status topMostFunction(bool gonna_fail)
{
    if(auto result = thisFunctionUsesBuildUpString(gonna_fail)) {
        ...
        return base::Status::Error(base::ErrorCode::INVALID_ARGUMENT);
    }
}
} // namespace
