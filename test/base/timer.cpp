...
constexpr std::int64_t milli_fault = 10;
...
BOOST_AUTO_TEST_CASE(time_usage2)
{
    ...
    std::this_thread::sleep_for(std::chrono::microseconds(213));
...
BOOST_AUTO_TEST_CASE(time_constructor_copy)
{
    base::Timer timer1;
    timer1.start();
    std::this_thread::sleep_for(std::chrono::microseconds(149));
...
BOOST_AUTO_TEST_CASE(time_elapsed_seconds)
{
    base::Timer timer;
    timer.start();
    std::this_thread::sleep_for(std::chrono::microseconds(1149));
    BOOST_CHECK(abs(timer.elapsedSeconds() - 1.15) < 0.01);
}