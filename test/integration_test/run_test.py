...
# TODO: implement logger insted print
# TODO: make script multi-platform
# TODO: make annotation to check and registrate test runners???
...
def run_registered_test_cases(node_exec_path, rpc_client_exec_path):
    ...
    for registered_case in tc.registered_test_cases:
        ...
        try:
            return_code = registered_test_case_runner(
...
if __name__ == "__main__":
    candiadte_folders = glob.glob(os.path.join(
        WORK_DIR, "..", "..", "*build*/"), recursive=False)
    ...
    return_code = run_registered_test_cases(
        node_exec_path, rpc_client_exec_path)
    exit(return_code)
