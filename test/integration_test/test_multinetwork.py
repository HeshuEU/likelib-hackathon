...
_work_dir = os.path.join(os.path.dirname(
    os.path.abspath(__file__)), "test_multinetwork")
...
def node_run(node_exec_path, node_info, another_nodes=[]):
    ...
    try:
        node_pipe = subprocess.run(
            [node_exec_path, "--config", _node_config_file], capture_output=True, timeout=test_timeout)
    except Exception:
        pass
    ...
    if not os.path.isfile(log_path):
        print(f"The log file for the node " +
              node_info.address + " does not exist")
        exit(1)
    ...
    for i in another_nodes:
        if (f"Connection established: " + i.address) not in log_content:
            print(f"Node with address " + node_info.address +
                  " did not connection the node with address " + i.address)
            exit(1)
...
#     node_process1 = mp.Process(target=node_run, args=[
#                                node_exec_path, node_inf1])
#     node_process2 = mp.Process(target=node_run, args=[
#                                node_exec_path, node_inf2, [node_inf1]])
#     node_process3 = mp.Process(target=node_run, args=[
#                                node_exec_path, node_inf3, [node_inf2, node_inf1]])
