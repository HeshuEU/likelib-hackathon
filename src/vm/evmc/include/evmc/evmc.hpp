...
{
    return { { { byte(s, 0),  byte(s, 1),  byte(s, 2),  byte(s, 3),  byte(s, 4),  byte(s, 5),  byte(s, 6),
...
class result : private evmc_result
{
  public:
    ...
    explicit result(evmc_result const& res) noexcept
      : evmc_result{ res }
    {}
...
class HostContext : public HostInterface
{
  ...
  public:
    ...
    bool account_exists(const address& address) const noexcept final { return host->account_exists(context, &address); }
    ...
    uint256be get_balance(const address& address) const noexcept final { return host->get_balance(context, &address); }

    size_t get_code_size(const address& address) const noexcept final { return host->get_code_size(context, &address); }
    ...
    result call(const evmc_message& message) noexcept final { return result{ host->call(context, &message) }; }
    ...
    bytes32 get_block_hash(int64_t number) const noexcept final { return host->get_block_hash(context, number); }
...
class VM
{
  public:
    ...
    explicit VM(evmc_vm* vm) noexcept
      : m_instance{ vm }
    {}
    ...
    evmc_capabilities_flagset get_capabilities() const noexcept { return m_instance->get_capabilities(m_instance); }
...
inline VM::VM(evmc_vm* vm, std::initializer_list<std::pair<const char*, const char*>> options) noexcept
  : m_instance{ vm }
{
...
struct hash<evmc::bytes32>
{
    ...
    constexpr size_t operator()(const evmc::bytes32& s) const noexcept
    {
        ...
        using namespace fnv;
        return static_cast<size_t>(
          fnv1a_by64(fnv1a_by64(fnv1a_by64(fnv1a_by64(fnv::offset_basis, load64be(&s.bytes[0])), load64be(&s.bytes[8])),
                                load64be(&s.bytes[16])),
                     load64be(&s.bytes[24])));
    }
};
