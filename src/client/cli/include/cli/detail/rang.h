...
{
#if defined(OS_LINUX) || defined(OS_MAC)
    static constexpr const char* Terms[] = { "ansi",  "color", "console", "cygwin", "gnome",  "konsole", "kterm",
    ...
    static const bool result = std::any_of(
      std::begin(Terms), std::end(Terms), [&](const char* term) { return std::strstr(env_p, term) != nullptr; });
...
template<typename T>
using enableStd = typename std::enable_if<std::is_same<T, rang::style>::value || std::is_same<T, rang::fg>::value ||
...
template<typename T>
using enableControl = typename std::enable_if<std::is_same<T, rang::control>::value, std::ostream&>::type;
}
...
inline rang_implementation::enableStd<T> operator<<(std::ostream& os, T const value)
{
    ...
    return (os.iword(rang_implementation::getIword()) ||
            ((rang_implementation::supportsColor()) && (rang_implementation::isTerminal(osbuf)))) ?
