  ...
  private:
    void Feed(char c)
    {
        if (std::isprint(c))
            ...
            std::cout << "0x" << std::hex << static_cast<int>(c) << std::dec << std::endl;
        /*
                switch ( c )
                {
                case 0: break;
...
{
  public:
    CliTelnetSession(boost::asio::ip::tcp::socket _socket,
                     Cli& _cli,
...
{
  public:
    CliTelnetServer(detail::asio::BoostExecutor::ContextType& ios,
                    unsigned short port,
                    Cli& _cli,
                    std::size_t _historySize = 100)
      : detail::Server(ios, port)
      , cli(_cli)
