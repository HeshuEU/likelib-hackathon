...
template<>
struct TypeDesc<signed char>
{
    static const char* Name() { return "<signed char>"; }
};
template<>
struct TypeDesc<short>
{
    static const char* Name() { return "<short>"; }
};
template<>
struct TypeDesc<unsigned short>
{
...
template<>
struct TypeDesc<long long>
{
    static const char* Name() { return "<long long>"; }
};
template<>
struct TypeDesc<unsigned long long>
{
...
class Cli
{
    ...
    class OutStream
    {
      ...
      public:
        ...
        using StandardEndLine = std::function<void(CoutType&)>;
      ...
      private:
        void Register(std::ostream& o) { ostreams.push_back(&o); }
    ...
    using ExitActionFn = std::function<void(std::ostream&)>;

  public:
    explicit Cli(std::unique_ptr<Menu>&& _rootMenu,
                 ExitActionFn _exitAction = {},
    ...
    void setExitAction(ExitActionFn action) { exitAction = action; }
    ...
    void StoreCommands(const std::vector<std::string>& cmds) { globalHistoryStorage->Store(cmds); }

    std::vector<std::string> GetCommands() const { return globalHistoryStorage->Commands(); }

  private:
    std::unique_ptr<HistoryStorage> globalHistoryStorage;
    std::unique_ptr<Menu> rootMenu; // just to keep it alive
    ExitActionFn exitAction;
};
  ...
  public:
    ...
    virtual std::vector<std::string> GetCompletionRecursive(const std::string& line) const
    {
        ...
        if (name.rfind(line, 0) == 0)
            return { name }; // name starts_with line
        else
            return {};
    }
...
class CliSession
{
  public:
    ...
    std::string PreviousCmd(const std::string& line) { return history.Previous(line); }

    std::string NextCmd() { return history.Next(); }
  ...
  private:
    struct Descriptor
    {
        ...
        void Remove()
        {
            ...
            if (scmd && scmds) {
                auto i =
                  std::find_if(scmds->begin(), scmds->end(), [&](const auto& c) { return c.get() == scmd.get(); });
                if (i != scmds->end())
                    scmds->erase(i);
            }
        }
...
class Menu : public Command
{
  public:
    ...
    template<typename F>
    [[deprecated("Use the method Insert instead")]] void Add(const std::string& cmdName,
                                                             F f,
                                                             const std::string& help = "")
    {
    ...
    [[deprecated("Use the method Insert instead")]] void Add(std::unique_ptr<Command>&& cmd)
    {
        std::shared_ptr<Command> s(std::move(cmd));
        cmds->push_back(s);
    }

    [[deprecated("Use the method Insert instead")]] void Add(std::unique_ptr<Menu>&& menu)
    {
    ...
    bool ScanCmds(const std::vector<std::string>& cmdLine, CliSession& session)
    {
        ...
        for (auto& cmd : *cmds)
            if (cmd->Exec(cmdLine, session))
                return true;
        if (parent && parent->Exec(cmdLine, session))
            return true;
        return false;
    }

    std::string Prompt() const { return Name(); }
  ...
  private:
    ...
    void Add(const std::string& name,
             const std::string& help,
             F& f,
             R (F::*mf)(A1, A2, A3, A4, std::ostream& out) const);
#endif // CLI_DEPRECATED_API

    template<typename F, typename R, typename... Args>
    CmdHandler Insert(const std::string& name,
                      const std::string& help,
                      const std::vector<std::string>& parDesc,
                      F& f,
                      R (F::*)(std::ostream& out, Args...) const);
    ...
    CmdHandler Insert(const std::string& name,
                      ...
                      const std::vector<std::string>& parDesc,
                      F& f,
                      R (F::*)(std::ostream& out, const std::vector<std::string>&) const);
    ...
    CmdHandler Insert(const std::string& name,
                      ...
                      const std::vector<std::string>& parDesc,
                      F& f,
                      R (F::*)(std::ostream& out, std::vector<std::string>) const);
...
//class FuncCmd : public Command
{
  public:
    ...
    FuncCmd(const std::string& _name, std::function<void(std::ostream&)> _function, const std::string& desc = "")
      : Command(_name)
      , function(_function)
      , description(desc)
    {}
    ...
    void Help(std::ostream& out) const override { out << " - " << Name() << "\n\t" << description << "\n"; }
...
class FuncCmd1 : public Command
{
  public:
    ...
    FuncCmd1(const std::string& _name, std::function<void(T, std::ostream&)> _function, const std::string& desc = "")
      : Command(_name)
      , function(_function)
      , description(desc)
    {}
...
class FuncCmd2 : public Command
{
  public:
    ...
    FuncCmd2(const std::string& _name,
    ...
    void Help(std::ostream& out) const override
    {
        out << " - " << Name() << " " << TypeDesc<T1>::Name() << " " << TypeDesc<T2>::Name() << "\n\t" << description
            << "\n";
    }
...
class FuncCmd3 : public Command
{
  public:
    ...
    FuncCmd3(const std::string& _name,
...
class FuncCmd4 : public Command
{
  public:
    ...
    FuncCmd4(const std::string& _name,
    ...
    {
        out << " - " << Name() << " " << TypeDesc<T1>::Name() << " " << TypeDesc<T2>::Name() << " "
            << TypeDesc<T3>::Name() << " " << TypeDesc<T4>::Name() << "\n\t" << description << "\n";
    }
...
class FreeformCommand : public Command
{
  public:
    ...
    FreeformCommand(const std::string& _name,
                    F fun,
...
inline void CliSession::Feed(const std::string& cmd)
{
    ...
    if (!found)
        found = current->ScanCmds(std::move(strs), *this); // last use of strs
...
{
    out << beforePrompt << current->Prompt() << afterPrompt << "> " << std::flush;
}
...
inline std::vector<std::string> CliSession::GetCompletions(std::string currentLine) const
{
    ...
    currentLine.erase(currentLine.begin(),
                      std::find_if(currentLine.begin(), currentLine.end(), [](int ch) { return !std::isspace(ch); }));
...
template<typename F, typename R, typename A1, typename A2, typename A3, typename A4>
void Menu::Add(const std::string& name,
               const std::string& help,
               F& f,
               R (F::*)(A1, A2, A3, A4, std::ostream& out) const)
{
...
CmdHandler Menu::Insert(const std::string& cmdName,
                        ...
                        const std::vector<std::string>& parDesc,
                        F& f,
                        R (F::*)(std::ostream& out, const std::vector<std::string>& args) const)
{
...
CmdHandler Menu::Insert(const std::string& cmdName,
                        ...
                        const std::vector<std::string>& parDesc,
                        F& f,
                        R (F::*)(std::ostream& out, std::vector<std::string> args) const)
{
    return Insert(std::make_unique<FreeformCommand<F>>(cmdName, f, help, parDesc));
}
