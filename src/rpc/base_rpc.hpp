...
class BaseRpc
{
  public:
    ...
    virtual std::string balance(const lk::Address& address) = 0;
    ...
    virtual std::tuple<OperationStatus, lk::Address, std::uint64_t> transaction_create_contract(
      lk::Balance amount,
    ...
    virtual std::tuple<OperationStatus, std::string, std::uint64_t> transaction_message_call(
      lk::Balance amount,
      const lk::Address& from_address,
